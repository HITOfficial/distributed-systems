...
#     def __init__(self) :
#         self.invokers={"A" : {"calls": 0, "values": []},
#                        "B" : {"calls": 0, "values": []},
#                        "C" : {"calls": 0, "values": []}}
#
#     def invoker_call(self, name):
...
#         return self.invokers[name]['calls']
#
#     def produce_new_value(self, name):
#         value = random.randint(5, 25)
#         self.invokers[name]['values'].append(value)
#         return value
#
...
#         calls = self.invoker_call(name)
#         # generate a random value and add it to the list of values
#         value = self.produce_new_value(name)
#         # return the state of that invoker
#         return calls, value
#
#     def get_invoker_values(self,name) :
#         return self.invokers[name]['values']
#
...
#     times_invoked, value_produced = ray.get(worker_invoker.invoke.remote(random_name_invoker))
#
#     print(f"Named caller: {random_name_invoker} called {times_invoked} producing value {value_produced}")
#
...
class PiActor:
    ...
    def get_calculations(self):
        return self.num_samples, self.in_circle


def compute_pi(num_samples, actors_num=4):
    actors = [PiActor.remote() for _ in range(actors_num)]
    for _ in range(num_samples):
        ray.get([actor.sample.remote() for actor in actors])

    pi_calculations = ray.get([actor.get_calculations.remote() for actor in actors])
    num_samples = sum(count for count, _ in pi_calculations)
    in_circle = sum(count for _, count in pi_calculations)
    return 4.0 * Fraction(in_circle, num_samples)


# 3.4 Run the code and observe the results

print(compute_pi(100 * 100,actors_num=10))
