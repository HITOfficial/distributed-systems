...
def bubble_sort_distributed(arr):
    arr = np.array(arr, copy=True) # read only ray, so need to copy
    return bubble_sort(arr)


def run_local_bubble_sort(arr):
    results = [bubble_sort(np.array(arr,copy=True)) for _ in range(os.cpu_count())]
    return results

def run_remote_bubble_sort(arr):
    results = ray.get([bubble_sort_distributed.remote(np.array(arr, copy=True)) for _ in range(os.cpu_count())])
    return results
...
t_local = time.time()
cProfile.run("run_local_bubble_sort(np.array(arr, copy=True))")
print('local bubble sort time', time.time() -t_local)

print('remote bubble sort')
t_remote = time.time()
cProfile.run("run_remote_bubble_sort(np.array(arr, copy=True))")
print('remote bubble sort time', time.time() -t_remote)
ray.shutdown()
