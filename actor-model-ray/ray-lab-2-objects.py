...
def fancy_dict(n):
    return {str(i): np.random.randint(1,1000,n) for i in range(n)}
...
arr_1_ref = ray.put(fancy_array(1_000_000))
dict_1_ref = ray.put(fancy_dict(500))


arr_squares_ref = arr_squares.remote(arr_1_ref)
dict_squares_ref = dict_squares.remote(dict_1_ref)

t  = time.time()
print("exec time for arr", time.time() - t ,'items processed' ,len(ray.get(arr_squares_ref)))
t = time.time()
print("exec time for dict", time.time() - t ,'items processed' , len(ray.get(dict_squares_ref)))
